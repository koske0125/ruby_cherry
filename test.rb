...
# puts/print/p/ppメソッド
# putsメソッドは改行を加え反映し、面数の内容/メソッドの戻り値をターミナルに出力する。戻り値はnilとなる。
...
# pメソッドはputs同様に改行を加える。文字列を出力した際に""込で出力される点が異なる。戻り値は引数で渡されたオブジェクト自体となる。

p 123
p "abc"

# ppメソッドは大きくて複雑な配列やハッシュ、オブジェクトを整形して出力する。また引数として渡したオブジェクトが戻り値となる。

arg_pp = [["日本","japan"],["アメリカ","america"],["インド","india"]]
