...
# メソッドの可変長引数
# 個数に制限のない引数のことを可変長引数と呼ぶ
# 自身で定義するメソッドで可変長引数を利用する場合には引数名に*を付ける

def hello(name,time,*msg)
  p "#{name}さん、今は#{time}時です。メッセージは#{msg.join("と")}です。"
end

hello("鈴木",5,["こんにちは","こんばんは"])

# *で配列同士を非破壊的に連結する
# []の中に*付きで別の配列を置くと要素が展開される

oo = [1,2,3]
p [oo] # この記述だと配列の中に配列として格納される
p [*oo] # 配列名に*を付与することで要素が展開されてから格納される

pp = [-1,0,*oo,4,5,6] # *を利用して別の配列を要素とする新しい配列を作成することが可能
p pp

# %記法を利用して文字列の配列を作成する
# 配列は[]を使用して作成することが多いが、文字列の配列を作成する際には%wを使用して作成することも可能
# %wを利用した場合、要素の区切りは","ではなく" "となり、文字列を""で囲む必要がなくなるため、コードを短く記述できるようになる

p ["日本","アメリカ","インド"] # []を利用したケース
p %w!日本 アメリカ インド! # %w!!(!!で文字列を囲むケース)
p %w(日本 アメリカ インド) # %w()で文字列を囲むケース

# 値にスペースを含めたい場合にはバックスラッシュを利用する

p %w(日本\ jp アメリカ\ us インド\ ind)
